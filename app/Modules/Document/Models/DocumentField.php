...
class DocumentField extends Model
{
    ...
    {
        static::creating(fn (DocumentField $model) => $model->created_by = $model->updated_by = auth()->id());
        static::updating(fn (DocumentField $model) => $model->updated_by = auth()->id());
    }
    ...
    {
        return $this->belongsToMany(Document::class, 'document_document_field')->withTimestamps();
    }
